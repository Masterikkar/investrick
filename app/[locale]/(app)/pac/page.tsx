...
export default async function PacPage() {
  ...
  // Una riga per PAC e per giorno: letta a blocchi per non fermarsi a 1000
  ...
  const righe: RigaTabella[] = (posizioni ?? [])
    .map((p) => {
      ...
      return {
        ...
        provenienza: p.contenitore_id ? nomePac.get(p.contenitore_id) ?? '—' : '—',
      }
    })
  ...
  return (
    <div>
      ...
      <h1 style={{ fontSize: 'var(--fs-h1)', marginTop: 4, marginBottom: 16, fontWeight: 500 }}>{tContenitori('pac')}</h1>
