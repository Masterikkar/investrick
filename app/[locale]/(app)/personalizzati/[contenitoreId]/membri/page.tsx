...
export default async function MembriGruppoPage({
  ...
  const [{ data: membriRaw }, { data: strumentiRaw }] = await Promise.all([
    ...
    supabase.from('strumenti').select('id, nome, ticker, isin, categoria').order('nome'),
  ])
  ...
  const selezionabili: StrumentoSelezionabile[] = strumenti
    .map((s) => ({ id: s.id, nome: s.nome, ticker: s.ticker, isin: s.isin, categoria: s.categoria, valore: aggregati.get(s.id)?.valore ?? 0 }))
  ...
  return (
    <div>
      ...
      <h1 style={{ fontSize: 'var(--fs-h1)', marginTop: 4, marginBottom: 16, fontWeight: 500 }}>{gruppo.nome}</h1>
      <p style={{ fontSize: 'var(--fs-body)', color: 'var(--text-secondary)', margin: '0 0 24px' }}>{t('intro')}</p>

      {errore === '1' && (
        <p style={{ color: 'var(--danger)', fontSize: 'var(--fs-body)', margin: '0 0 16px' }}>{t('erroreGenerico')}</p>
      )}
